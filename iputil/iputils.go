...
import (
	...
	"strconv"
	"strings"
	strings_util "utils/strings_util"
)
...
func IsIPv4(ips ...interface{}) bool {
	for _, ip := range ips {
		...
		case string:
			...
			isIP4 := parsedIP != nil && parsedIP.To4() != nil && strings_util.ContainsAny(ipv, ".")
			if !isIP4 {
				return false
			}
		case net.IP:
			isIP4 := ipv != nil && ipv.To4() != nil && strings_util.ContainsAny(ipv.String(), ".")
			if !isIP4 {
				return false
			}
		}
...
func IsIPv6(ips ...interface{}) bool {
	for _, ip := range ips {
		...
		case string:
			parsedIP := net.ParseIP(ipv)
			isIP6 := parsedIP != nil && parsedIP.To16() != nil && strings_util.ContainsAny(ipv, ":")
			if !isIP6 {
				return false
			}
		case net.IP:
			isIP6 := ipv != nil && ipv.To16() != nil && strings_util.ContainsAny(ipv.String(), ":")
			if !isIP6 {
				return false
			}
		}
